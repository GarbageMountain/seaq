...
describe('small collection', () => {
  test('getProperty', () => {
    const searchResults = seaq(Contacts, 'merv pre', ['givenName', 'familyName']);
...
describe('large collection', () => {
  test('getProperty', () => {
    const searchResults = seaq(ManyContacts as any, 'nath fe', ['givenName', 'familyName']);
    expect(searchResults).toHaveLength(1);
    expect(searchResults[0]).toMatchObject({ givenName: 'Natasha' });
  });

  test('getProperty', () => {
    const searchResults = seaq(ManyContacts as any, 'Natniel', ['givenName'], 0.5);
  ...
  test('getProperty', () => {
    const searchResults = seaq(ManyContacts as any, '', ['givenName', 'familyName']);
    expect(searchResults).toHaveLength(0);
  });
...
describe('extra features', () => {
  ...
  test('acronym bonus fuzzy', () => {
    const searchResults = seaq(['Hillsdale Michigan', 'historymi'], 'HiMi', undefined, 0.5);
    expect(searchResults).toHaveLength(2);
    expect(searchResults[0]).toBe('Hillsdale Michigan');
  });
});
